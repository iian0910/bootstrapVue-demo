      ...
      <div class="row">
        <div class="col-10 offset-1">
          ...
          <div class="d-flex justify-content-center">
            <b-pagination
              v-model="currentPage"
              :total-rows="newfilterData.length"
              :per-page="perPage"
              pills
              first-number
              last-number
...
export default class TablePagination extends Vue {
  ...
  // Computed
  get filterInput () {
    this.newfilterData = this.data.filter((item:Data) => item.taxCategory.match(this.filterText))
    this.newData = this.newfilterData.slice(this.perPage * this.currentPage - this.perPage, this.perPage * this.currentPage)
    // return this.data.filter((item:Data) => item.taxCategory.match(this.filterText))
    return this.newData
  }
