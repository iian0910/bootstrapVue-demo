  ...
  <div>
    <div class="container">
      ...
      <b-table striped hover :items="sliceDate">
        <template #cell(seq)="sliceDate">
          <div class="text-left">{{sliceDate.item.seq}}</div>
        </template>
        <template #cell(materialYear)="sliceDate">
          <div class="text-right">{{sliceDate.item.materialYear}}</div>
        </template>
        <template #cell(statisticsProject)="sliceDate">
          <div class="text-right">{{sliceDate.item.statisticsProject}}</div>
        </template>
        <template #cell(taxCategory)="sliceDate">
          <div class="text-right">{{sliceDate.item.taxCategory}}</div>
        </template>
        <template #cell(value)="sliceDate">
          <div class="text-left">{{sliceDate.item.value | currencyChange}}</div>
        </template>
        <template #cell(dataUnit)="sliceDate">
          <div class="text-right">{{sliceDate.item.dataUnit}}</div>
        </template>
      </b-table>
      <!-- Pagination -->
      <b-pagination
