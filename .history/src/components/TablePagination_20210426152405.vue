      ...
      <div class="row">
        <div class="col-10 offset-1">
          ...
          <b-table
            ...
            :items="filterInput"
          >
            <template #cell(seq)="filterInput">{{filterInput.item.seq}}</template>
            <template #cell(materialYear)="filterInput">{{filterInput.item.materialYear}}</template>
            <template #cell(statisticsProject)="filterInput">{{filterInput.item.statisticsProject}}</template>
            <template #cell(taxCategory)="filterInput">{{filterInput.item.taxCategory}}</template>
            <template #cell(value)="filterInput">
              <div class="text-left">{{filterInput.item.value | currencyChange}}</div>
            </template>
            <template #cell(dataUnit)="filterInput">{{filterInput.item.dataUnit}}</template>
          </b-table>
          <!-- Pagination -->
...
export default class TablePagination extends Vue {
  ...
  // Computed
  // this.data.filter((item:any) => { item.value.match(this.filterText) })
  get filterInput () {
    return this.data.filter((item:any) => { item.value.match(this.filterText) })
  }
