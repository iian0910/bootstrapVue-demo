...
export default class TablePagination extends Vue {
  ...
  private filterData (currentPage: number) {
    this.sliceData = this.filterInput.slice(this.perPage * currentPage - this.perPage, this.perPage * currentPage)
  }

  // Computed
  get filterInput () {
    return this.sliceData.filter((item:Data) => item.taxCategory.match(this.filterText)).slice(this.perPage * this.currentPage - this.perPage, this.perPage * this.currentPage)
  }
