...
export default class TablePagination extends Vue {
  ...
  // Computed
  get filterInput () {
    const filterData = this.data.filter((item:Data) => item.taxCategory.match(this.filterText))
    this.newData = filterData.slice(this.perPage * this.currentPage - this.perPage, this.perPage * this.currentPage)
